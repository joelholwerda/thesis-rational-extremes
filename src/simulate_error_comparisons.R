# Run simulations by combining the using utility-weighed sampling functions (no encoding) for the complete model, no bias correction, and representative sampling.
...
simulate_error_comparisons <- function(n_simulations, utilities, average_utility, sd_noise, n_samples) {
    ...
    parameters_representative <- tibble(
        n_samples = n_samples %>% 
            rep(length(sd_noise) * n_simulations * 10),
        sd_noise = sd_noise %>% 
            rep(each = length(.env$n_samples)) %>% 
            rep(n_simulations * 10)
    )
    ...
    simulations$uws <- pmap_dfr(
        ...
        # Generate n_simulations estimates using the utility-weighted sampling model without bias correction
    simulations$uncorrected <- pmap_dfr(
        parameters_uws,
        # Generate estimates using the utility-weighted sampling functions
        function(average_utility, n_samples, sd_noise) {
            # Generate utilities if `utilities` contains a random function
    ...
    simulations$summary <- bind_rows(
        list(
            uws = simulations$uws, 
            uncorrected = simulations$uncorrected,
